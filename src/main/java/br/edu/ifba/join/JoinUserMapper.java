...
public class JoinUserMapper extends Mapper<Object, Text, Text, Text> {
	...
	protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        ...
        System.out.println("USER: O que tem em linha: " + linha);

        if (xmlAttributes.size() > 0) {
            if (xmlAttributes.containsKey("Id") && xmlAttributes.containsKey("DisplayName")) {
                this.outKey.set(xmlAttributes.get("Id"));
                this.outValue.set("P" + xmlAttributes.get("DisplayName"));
